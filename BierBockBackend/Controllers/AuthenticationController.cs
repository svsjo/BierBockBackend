...
public class AuthenticationController : ControllerBase
{
    ...
    public RequestStatus<object> Register(RegisterUser registerUser)
    {
        if (!registerUser.IsUserNameValid)
            return new RequestStatus<object> { Status = Status.Error, ErrorCode = ErrorCodes.invalid_username };

        if (_databaseContext.GetUsers().Any(x => x.UserName == registerUser.UserName))
            return new RequestStatus<object> { Status = Status.Error, ErrorCode = ErrorCodes.username_taken };

        if (_databaseContext.GetUsers().Any(x => x.Email == registerUser.Email))
            return new RequestStatus<object> { Status = Status.Error, ErrorCode = ErrorCodes.mail_taken };

        if (!registerUser.IsVornameValid)
            return new RequestStatus<object> { Status = Status.Error, ErrorCode = ErrorCodes.invalid_firstname_format };

        if (!registerUser.IsNachnameValid)
            return new RequestStatus<object> { Status = Status.Error, ErrorCode = ErrorCodes.invalid_surname_format };

        if (!registerUser.IsPasswordValid)
            return new RequestStatus<object> { Status = Status.Error, ErrorCode = ErrorCodes.invalid_password_format };

        if (!registerUser.IsEmailValid)
            return new RequestStatus<object> { Status = Status.Error, ErrorCode = ErrorCodes.invalid_email_format };

        if (!registerUser.IsBirthdateValid)
            return new RequestStatus<object> { Status = Status.Error, ErrorCode = ErrorCodes.invalid_birthdate_format };
    ...
    public RequestStatus<object> CreateToken(AuthUser user)
    {
        ...
        var tokenDescriptor = new SecurityTokenDescriptor
        {
            Subject = new ClaimsIdentity(new[]
            {
                ...
                new Claim(IdentityData.AdminUserClaim, userMatch.IsAdmin.ToString())
            }),
